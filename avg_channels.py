        ...
        for a in data:
            if float(a["ID"]) == float(b["ParentID"]):
                ...
                if (float(a["PM2_5Value"]) - float(b["PM2_5Value"])) >= 10:
                    ...
                    # print("Averaging")
                    x["PM2_5Value"] = ((float(a["PM2_5Value"]) + float(b["PM2_5Value"])) / 2)
                    x["v"] = average_round(a["v"], b["v"])
                    x["v1"] = average_round(a["v1"], b["v1"])
                    x["v2"] = average_round(a["v2"], b["v2"])
                    x["v3"] = average_round(a["v3"], b["v3"])
                    x["v4"] = average_round(a["v4"], b["v4"])
                    x["v5"] = average_round(a["v5"], b["v5"])
                    x["v6"] = average_round(a["v6"], b["v6"])
                # print(a, b, x)
                avg_data.append(x)
                # print(avg_data)
...
for monitor in avg_data:
    sql = "INSERT INTO cur_avg_data (ID, ParentID, Label, THINGSPEAK_PRIMARY_ID," + \
